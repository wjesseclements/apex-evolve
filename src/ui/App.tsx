...
import { DEFAULT_PHYSICS } from '../sim/config.ts';
...
export function App() {
  ...
  const sim = useSimLoop(canvasRef, TRAINING_TRACK, DEFAULT_PHYSICS, controlsRef);
  const { reset, toggleDebug } = sim;
  ...
  return (
    <main className="app">
      <header className="app__header">
        <h1>apex-evolve</h1>
        <p>Slice 0 — drive the car with the arrow keys. Neuroevolution arrives in later slices.</p>
      </header>
      <div className="app__body">
        ...
        <div className="side">
          <Hud hud={sim.hud} debug={sim.debug} />
